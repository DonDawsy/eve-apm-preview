...
void ColorButtonBinding::saveToConfig() { m_configSetter(m_currentColor); }
...
QWidget *ColorButtonBinding::widget() const { return m_button; }
...
StringListTableBinding::StringListTableBinding(QTableWidget *table, int column,
                                               Getter configGetter,
                                               Setter configSetter,
                                               QStringList defaultValue)
    : m_table(table), m_column(column), m_configGetter(configGetter),
      m_configSetter(configSetter), m_defaultValue(defaultValue),
...
void StringListTableBinding::loadFromConfig() {
  ...
  for (const QString &item : items) {
    if (isProcessNamesTable &&
        item.compare("exefile.exe", Qt::CaseInsensitive) == 0) {
      continue;
    }
...
QWidget *StringListTableBinding::widget() const { return m_table; }
...
CharacterHotkeyTableBinding::CharacterHotkeyTableBinding(QTableWidget *table,
                                                         Getter configGetter,
                                                         Setter configSetter)
    : m_table(table), m_configGetter(configGetter),
...
void CharacterHotkeyTableBinding::loadFromConfig() {
  ...
  QHash<QString, QVector<HotkeyBinding>> multiHotkeys =
      mgr ? mgr->getAllCharacterMultiHotkeys()
  ...
  for (auto it = multiHotkeys.constBegin(); it != multiHotkeys.constEnd();
       ++it) {
    int row = m_table->rowCount();
    m_table->insertRow(row);
    ...
    for (const HotkeyBinding &binding : it.value()) {
      combinations.append(HotkeyCombination(binding.keyCode, binding.ctrl,
...
void CharacterHotkeyTableBinding::saveToConfig() {
  ...
  for (int row = 0; row < m_table->rowCount(); ++row) {
    QLineEdit *nameEdit =
        qobject_cast<QLineEdit *>(m_table->cellWidget(row, 0));
      ...
      if (!charName.isEmpty()) {
        ...
        if (!combinations.isEmpty()) {
          ...
          for (const HotkeyCombination &comb : combinations) {
            bindings.append(HotkeyBinding(comb.keyCode, comb.ctrl, comb.alt,
                                          comb.shift, true));
          }
...
bool CharacterHotkeyTableBinding::hasChanged() const {
  ...
  for (auto it = currentMulti.constBegin(); it != currentMulti.constEnd();
       ++it) {
    ...
    const QVector<HotkeyBinding> &initialBindings =
        m_initialMultiHotkeys[charName];
...
QWidget *CharacterHotkeyTableBinding::widget() const { return m_table; }
...
CycleGroupTableBinding::CycleGroupTableBinding(QTableWidget *table,
    ...
    : m_table(table), m_configGetter(configGetter),
      m_configSetter(configSetter), m_buttonConnector(buttonConnector) {}
...
void CycleGroupTableBinding::loadFromConfig() {
  ...
  for (auto it = groups.constBegin(); it != groups.constEnd(); ++it) {
    ...
    if (!it.value().backwardBindings.isEmpty()) {
      ...
      backwardCapture->setHotkey(
          it.value().backwardBinding.keyCode, it.value().backwardBinding.ctrl,
          it.value().backwardBinding.alt, it.value().backwardBinding.shift);
    }
    ...
    QObject::connect(clearBackwardButton, &QPushButton::clicked,
    ...
    m_table->setCellWidget(row, 2, backwardHotkeyWidget);
    ...
    if (!it.value().forwardBindings.isEmpty()) {
      ...
      forwardCapture->setHotkey(
          it.value().forwardBinding.keyCode, it.value().forwardBinding.ctrl,
          it.value().forwardBinding.alt, it.value().forwardBinding.shift);
    }
    ...
    QObject::connect(clearForwardButton, &QPushButton::clicked,
    ...
    m_table->setCellWidget(row, 3, forwardHotkeyWidget);
    ...
    QString checkboxStyle =
        QString("QCheckBox {"
                ...
                "url(data:image/"
                "svg+xml;base64,"
                "PHN2ZyB3aWR0aD0iMTIiIGhlaWdodD0iMTAiIHhtbG5zPSJodHRwOi8vd3d3Ln"
                "czLm9yZy8yMDAwL3N2ZyI+"
                "PHBhdGggZD0iTTEgNUw0IDhMMTEgMSIgc3Ryb2tlPSIjZmZmZmZmIiBzdHJva2"
                "Utd2lkdGg9IjIiIGZpbGw9Im5vbmUiLz48L3N2Zz4=);"
                "}"
...
void CycleGroupTableBinding::saveToConfig() {
  ...
  for (int row = 0; row < m_table->rowCount(); ++row) {
    ...
    QPushButton *charactersButton =
        qobject_cast<QPushButton *>(m_table->cellWidget(row, 1));
    QWidget *backwardHotkeyWidget = m_table->cellWidget(row, 2);
    QWidget *forwardHotkeyWidget = m_table->cellWidget(row, 3);
    QWidget *checkboxContainer =
        qobject_cast<QWidget *>(m_table->cellWidget(row, 4));
    QWidget *noLoopContainer =
        qobject_cast<QWidget *>(m_table->cellWidget(row, 5));
    ...
    if (nameEdit && charactersButton && backwardCapture && forwardCapture &&
        checkboxContainer) {
      ...
      bool includeNotLoggedIn = includeNotLoggedInCheck
                                    ? includeNotLoggedInCheck->isChecked()
                                    : false;

      QCheckBox *noLoopCheck =
          noLoopContainer ? noLoopContainer->findChild<QCheckBox *>() : nullptr;
      ...
      if (!groupName.isEmpty() && !charactersList.isEmpty()) {
        ...
        if (!characters.isEmpty()) {
          QVector<HotkeyCombination> forwardCombinations =
              forwardCapture->getHotkeys();
          QVector<HotkeyBinding> forwardBindings;
          for (const HotkeyCombination &comb : forwardCombinations) {
            forwardBindings.append(HotkeyBinding(comb.keyCode, comb.ctrl,
                                                 comb.alt, comb.shift,
                                                 comb.keyCode != 0));
          }

          QVector<HotkeyCombination> backwardCombinations =
              backwardCapture->getHotkeys();
          QVector<HotkeyBinding> backwardBindings;
          for (const HotkeyCombination &comb : backwardCombinations) {
            backwardBindings.append(HotkeyBinding(comb.keyCode, comb.ctrl,
                                                  comb.alt, comb.shift,
                                                  comb.keyCode != 0));
          }

          HotkeyBinding forwardBinding =
              !forwardBindings.isEmpty()
                  ? forwardBindings.first()
                  : HotkeyBinding(0, false, false, false, false);

          HotkeyBinding backwardBinding =
              !backwardBindings.isEmpty()
...
bool CycleGroupTableBinding::hasChanged() const {
  ...
  for (auto it = current.constBegin(); it != current.constEnd(); ++it) {
    ...
    if (currentGroup.forwardBindings.size() !=
        initialGroup.forwardBindings.size()) {
      return true;
    }
    for (int i = 0; i < currentGroup.forwardBindings.size(); ++i) {
      if (!(currentGroup.forwardBindings[i] ==
            initialGroup.forwardBindings[i])) {
        return true;
      }
    }

    if (currentGroup.backwardBindings.size() !=
        initialGroup.backwardBindings.size()) {
      return true;
    }
    for (int i = 0; i < currentGroup.backwardBindings.size(); ++i) {
      if (!(currentGroup.backwardBindings[i] ==
            initialGroup.backwardBindings[i])) {
        return true;
      }
    }
  }
...
QWidget *CycleGroupTableBinding::widget() const { return m_table; }
...
void HotkeyCaptureBinding::loadFromConfig() {
  ...
  m_widget->setHotkey(current.keyCode, current.ctrl, current.alt,
                      current.shift);
}
...
void HotkeyCaptureBinding::saveToConfig() {
  ...
  if (!combinations.isEmpty()) {
    ...
    HotkeyBinding current(first.keyCode, first.ctrl, first.alt, first.shift,
                          first.keyCode != 0);
...
QWidget *HotkeyCaptureBinding::widget() const { return m_widget; }

CharacterColorTableBinding::CharacterColorTableBinding(
    QTableWidget *table, ColorUpdateFunc colorUpdateFunc,
    ButtonConnector buttonConnector)
    : m_table(table), m_colorUpdateFunc(colorUpdateFunc),
...
void CharacterColorTableBinding::reset() {
  Config &config = Config::instance();
  for (auto it = m_initialValue.constBegin(); it != m_initialValue.constEnd();
       ++it) {
    config.removeCharacterBorderColor(it.key());
  }
...
bool CharacterColorTableBinding::hasChanged() const {
  ...
  for (auto it = currentValue.constBegin(); it != currentValue.constEnd();
       ++it) {
    if (!m_initialValue.contains(it.key()) ||
        m_initialValue[it.key()] != it.value()) {
      return true;
    }
  }
...
QWidget *CharacterColorTableBinding::widget() const { return m_table; }

FontBinding::FontBinding(QComboBox *fontCombo, QSpinBox *sizeSpinBox,
                         Getter configGetter, Setter configSetter,
                         QFont defaultValue)
    : m_fontCombo(fontCombo), m_sizeSpinBox(sizeSpinBox),
      m_configGetter(configGetter), m_configSetter(configSetter),
...
void FontBinding::reset() {
  int index =
      m_fontCombo->findText(m_defaultValue.family(), Qt::MatchFixedString);
  if (index >= 0) {
    m_fontCombo->setCurrentIndex(index);
  }
...
QWidget *FontBinding::widget() const { return m_fontCombo; }

void SettingBindingManager::addBinding(
    std::unique_ptr<SettingBindingBase> binding) {
...
void SettingBindingManager::clear() { m_bindings.clear(); }
